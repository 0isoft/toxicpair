...
type Tier = "INTERN" | "JUNIOR" | "SENIOR";
type Problem = { id: number; title: string; difficulty: string; tier: Tier };
type TierProgress = { passed: number; total: number; neededForNext: number };
type Progress = {
  INTERN: TierProgress;
  JUNIOR: TierProgress;
  SENIOR: TierProgress;
...
const TIER_THEME: Record<
  Tier,
  { from: string; to: string; bar: string; emoji: string }
> = {
...
const TIER_TICKETS: Record<Tier, number> = { INTERN: 3, JUNIOR: 5, SENIOR: 7 };
...
function promotionCopy(prog: Progress | undefined) {
  if (!prog) return "";
...
function Badge({ text, tone }: { text: string; tone: "easy" | "medium" | "hard" | "neutral" }) {
  return <span className={`badge ${tone}`}>{text}</span>;
}

function TicketWallet({ tier }: { tier: Tier }) {
  const t = TIER_TICKETS[tier];
  return (
    <div className="ticketWallet">
      <span className="twIcon">🎟</span>
      <div className="twMeta">
        <div className="twLabel">Tickets</div>
        <div className="twValue">{t}</div>
      </div>
    </div>
  );
}
...
function ProgressBar({ value, total, color }: { value: number; total: number; color: string }) {
  ...
  return (
    <div className="progOuter">
      <div className="progInner" style={{ width: `${w}%`, backgroundColor: color }} />
    </div>
  );
}

function ProblemCard({ p, locked }: { p: Problem; locked: boolean }) {
  const tone =
  ...
  return (
    <div className={`probCard ${locked ? "locked" : ""}`}>
      {locked && <div className="lockVeil" />}
      <div className="probTop">
        <div className="probTitle">
          {locked ? (
            <span className="probText lockedTitle" title="Locked">
              {p.title}
            </span>
          ) : (
            <Link to={`/problems/${p.id}`} className="probLink">
              {p.title}
            </Link>
          )}
        </div>
        <div className="probIcon">{locked ? "🔒" : "➡️"}</div>
      </div>

      <div className="probBadges">
        <Badge text={p.difficulty} tone={tone as any} />
        <Badge text={p.tier} tone="neutral" />
      </div>
    </div>
  );
}
...
function TierTrack({
  ...
  return (
    <section className="tierSection">
      <div
        className="tierHeader"
        style={{ background: `linear-gradient(90deg, ${theme.from}, ${theme.to})` }}
      >
        <div className="tierHeadL">
          <span className="tierEmoji">{theme.emoji}</span>
          <div className="tierHeadMeta">
            <div className="tierTitle">{TIER_LABEL[tier]}</div>
            <div className="tierSub">{locked ? "Locked — earn your promotion to enter." : "Unlocked — pick a ticket and go!"}</div>
          </div>
        </div>
      ...
      <ProgressBar value={solved} total={total || 1} color={theme.bar} />

      {locked && (
        <div className="lockedMsg">
          This level is locked. Clear the promotion requirement in your current tier to unlock it.
        </div>
      )}

      <div className="cardGrid">
        {(problems ?? []).map((p) => (
          <ProblemCard key={p.id} p={p} locked={locked} />
        ))}

        {!locked && (problems?.length ?? 0) === 0 && (
          <div className="emptyCard">No tickets in this tier yet.</div>
        )}
        {locked && (problems?.length ?? 0) === 0 && (
          <div className="emptyCard muted">Locked content</div>
        )}
      </div>
    </section>
  );
...
export default function ProblemsList() {
  ...
  const fetchTier = async (tier: Tier) => {
    try {
      return await api<Problem[]>(`/problems?tier=${tier}`);
    } catch (err: any) {
  ...
  const currentTier: Tier | undefined = prog?.unlockedMax;
...
`}</style>
      ...
      >
        ...
        <div className="heroRow">
          <div>
            <div className="heroKicker">Career Ladder</div>
            <div className="heroTitle">
              {currentTier ? `${TIER_LABEL[currentTier]} Engineer` : "Loading…"}
            </div>
            <div className="heroSub">
              {loadingProg ? "Fetching your progress…" : errProg ? "Failed to load progress." : promotionCopy(prog)}
            </div>
          </div>
          {currentTier && <TicketWallet tier={currentTier} />}
        </div>
      </div>

      {/* Tier tracks */}
      <div className="sectionStack">
        <TierTrack tier="INTERN" problems={intern ?? []} locked={false} progress={prog?.INTERN} />
        <TierTrack
          tier="JUNIOR"
          problems={unlockedJunior ? (junior ?? []) : []}
          locked={!unlockedJunior}
          progress={prog?.JUNIOR}
        />
        <TierTrack
          tier="SENIOR"
          problems={unlockedSenior ? (senior ?? []) : []}
          locked={!unlockedSenior}
          progress={prog?.SENIOR}
        />
      </div>

      {(loadingIntern || loadingJunior || loadingSenior) && (
        <p className="hint" aria-live="polite">Loading tickets…</p>
      )}
    </main>
  );
}
