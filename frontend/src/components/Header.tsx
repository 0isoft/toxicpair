...
export default function Header() {
  ...
  // Call hooks unconditionally (no early returns)
  const isLanding = pathname === "/";

  const { data: me } = useQuery({
    queryKey: ["me"],
    queryFn: () => api<User>("/me"),
    enabled: !!token && !isLanding, // don't fetch when hidden
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
  ...
  // Also close dropdown when route changes
...
.navInner { display:flex; align-items:center; gap:12px; padding:10px 16px; min-width:0; position:relative; }
...
.spacer { flex:1; }
.userMenu { display:flex; align-items:center; gap:10px; position:relative; }
.userMenu .navlink { white-space:nowrap; }
...
.menuPanel { position:absolute; margin-top:8px; right:0; background:#fff; border:1px solid #e5e7eb; border-radius:12px; box-shadow:0 8px 24px rgba(0,0,0,.08); min-width:220px; z-index:50; }
...
@media (max-width: 560px) {
  ...
  .navLinks { grid-area: nav; justify-content: center; }
  .userMenu { grid-area: user; justify-content: end; }
  .spacer { display: none; }
  .navlink { padding: 6px 10px; font-size: 14px; }
}
`;

  // Hide on landing by rendering nothing AFTER hooks ran (hook order safe)
